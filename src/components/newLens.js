...
const NewLens = ({twitterHandle, setTwitterHandle}) => {
  ...
  const [lensImage, setLensImage] = useState("https://www.handiclubnimois.fr/wp-content/uploads/2020/10/blank-profile-picture-973460_1280.png");
  ...
  useEffect(() => {
    axios.get('http://127.0.0.1:5000/user/' + twitterHandle.replace('@', ''))
        .then(res => {
          ...
          setLensImage(res.data.user.profileImageUrl.replace('_normal', '_bigger'));
        })
  }, [])
  ...
  if (toggle){
    return (
      <Fragment> 
        ...
        <img src={lensImage} className="inline object-cover w-20 h-20 mr-2 rounded-full center"/>
        ...
        <label className="block text-gray-500 font-bold md:text-center mb-1 md:mb-0 pr-4">Your new Lens Handle</label>
          ...
          ></input>
          <label className="block text-gray-500 font-bold md:text-center mb-1 md:mb-0 pr-4">Your new Lens Bio</label>
          <input className="bg-gray-200 appearance-none border-2 border-gray-200 rounded w-full py-2 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-purple-500"
            id="inline-full-name"
            type="text" 
            value={lensBio}
    ...
    return(
      <Fragment>
      <form className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        <img src={lensImage} class="inline object-cover w-20 h-20 mr-2 rounded-full"/>
